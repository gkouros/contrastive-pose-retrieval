...
def load_occluders(pascal_voc_root_path, excluded_classes=()):
    occluders = []
    ...
    for annotation_path in tqdm(annotation_paths, desc='Loading occluders'):
        ...
        for i_obj, obj in enumerate(xml_root.findall('object')):
            ...
            is_excluded = obj.find('name').text in excluded_classes
...
def create_occluders_file(occluders_path, save_path, exclude=()):
    occluders = np.asanyarray(
        load_occluders(occluders_path, excluded_classes=exclude), dtype=object)
    np.savez(save_path, occluders, allow_pickle=True)


def load_occluders_file(path):
    occluders = np.load(path, allow_pickle=True)
    return occluders['arr_0']
