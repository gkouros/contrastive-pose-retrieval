...
flags.DEFINE_integer('num_workers', 4, 'Set the number of cpu workers')
...
flags.DEFINE_string('dataset_path',
                    '/media/gkouros/Elements SE/Datasets/pascal3d+',
                    "The path to PASCAL3D+ dataset")
flags.DEFINE_enum('positive_type', 'rendering',
...
flags.DEFINE_float('subset_ratio', 1.0,
...
flags.DEFINE_enum('labelling_method', 'azim',
...
flags.DEFINE_float('pose_positive_threshold', 5, 'The angle in degrees that '
...
# Architecture Flags
flags.DEFINE_enum('loss', 'triplet',
...
flags.DEFINE_integer('embedding_size', 128, 'The size of the final embeddings')
flags.DEFINE_boolean('multimodal', True, 'Dual net for anchors and posnegs')
flags.DEFINE_enum('backbone', 'resnet18', ['resnet18', 'resnet50'],
                  'Select backbone between resnet18 and resnet50')

# optimization params
flags.DEFINE_float('margin', 0.2, 'Margin of triplet loss function')
...
flags.DEFINE_enum('type_of_triplets', 'hard', ['all', 'hard', 'semihard'],
                  'Miner selection of hard, semihard, or all triplets')
flags.DEFINE_float('trunk_lr', 0.00005, 'Learning rate for the trunk')
flags.DEFINE_float('trunk_wd', 0.00005, 'Weight decay for the trunk')
flags.DEFINE_float('embedder_lr', 0.0005, 'Learning rate for the embedder')
flags.DEFINE_float('embedder_wd', 0.0005, 'Weight decay for the embedder')
flags.DEFINE_enum('lr_scheduler', 'plateau', ['none', 'plateau', 'exp', 'step'],
                  'Sets the type of lr scheduler')
flags.DEFINE_integer('lr_decay_patience', 5,
...
flags.DEFINE_boolean('warm_start', True, 'If true, only embedder is trained '
                     'in the first epoch')
flags.DEFINE_integer('batch_size', 32, 'Specify the batch size for training')
flags.DEFINE_integer('patience', 20, 'Epoches in plateau before termination')
flags.DEFINE_integer('k', 32, 'k of k-NN for evaluating performance. '
                     'Valid values: 0 -> None, 1 -> max_bin_count, '
                     '>1 -> same value, -1 -> batch_size')
flags.DEFINE_enum('primary_metric', 'precision_at_1',
...
def main():
    ...
    train_dataset, val_dataset, _ = \
        get_pascal3d_train_val_test_datasets(
            ...
            synthetic_occlusion_scale=FLAGS.occlusion_scale,
